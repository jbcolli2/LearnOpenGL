...
float computeShadowCoeff(vec3 frag2Light)
{
    ...
    (
        ...
        vec3( 0, 1, 1), vec3( 0, -1, 1), vec3( 0, -1, -1), vec3( 0, 1, -1), vec3(0,0,0)
    );
    int numSamples = 21;
    float bias = .05;
    float diskRadius = .005;
    ...
    {
        float closestDepth = texture(shadowMap, frag2Light + sampleOffsetDirections[ii] * diskRadius).r;
        closestDepth *= farPlane;
        
        shadow += FragDepth - bias > closestDepth ? 1.0 : 0.0;
    }
